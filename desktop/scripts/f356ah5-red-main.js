 ...
 *   • FIX (1.0.234) — worktree. Clique → 1 POST confirmClientSend (com Authorization) → botão "Envio
...
app.whenReady().then(async () => {
  ...
  const VERSION = process.env.H5_VERSION || "1.0.234";
  ...
  /* ══════════════════════ FIX (1.0.234) — CLIQUE REAL no botão ══════════════════════ */
  const F = await bootAsar(fixAsar, "fix");
  recInv("R00 renderer FIX (1.0.234) bootado (Social logada; saveTask/openSendClientModal/wfConfirmClientSend vivos)",
