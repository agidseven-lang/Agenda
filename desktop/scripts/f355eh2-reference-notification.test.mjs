...
/* ============ A. IDENTIDADE (RE-PINADO F3.5.6A: candidata 1.0.229; contrato VISUAL da referência CONGELADO) ============ */
console.log('— A) identidade da candidata —');
ok('A1 package.json 1.0.229 (RE-PINADO F3.5.6A)', /"version":\s*"1\.0\.229"/.test(PKG));
...
ok('A3 package-lock 1.0.229 (2x — RE-PINADO F3.5.6A)', (LOCK.match(/"version":\s*"1\.0\.229"/g) || []).length >= 2);
