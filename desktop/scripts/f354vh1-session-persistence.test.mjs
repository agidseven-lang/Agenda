...
// [ATUALIZADA F3.5.5C-H1] na 1.0.221 (hotfix P0 da restauração de sessão no boot: auth-core self() com autoridade do servidor + renderer no_bridge/guarda offline — a MÁQUINA de estados da 1.0.213 segue preservada).
// A versão do pacote agora é 1.0.221.
ok('39 versão 1.0.221 no package.json (persistência 1.0.213 preservada)', JSON.parse(fs.readFileSync(path.resolve(__dirname,'..','package.json'),'utf8')).version==='1.0.221');
