/* F3.5.6A-H5 — PRELOAD do RED do BOTÃO CONFIRMAR (reusa a base H4; + toggle __TEAMFAIL p/ falha de servidor).
 * F3.5.6A-H4 base — PRELOAD do RED do CAMINHO REAL DE CRIAÇÃO. Diferente da H3 (que semeou o doc),
...
window.__TEAMFAIL = false; // F3.5.6A-H5: quando true, /team-action responde erro (falha de servidor no confirm)
...
(function () {
  ...
  window.fetch = function (url, opts) {
    ...
    if (u.indexOf('/team-action') >= 0) {
      if (window.__TEAMFAIL) { try { window.__TEAM_POSTS.push({ url: u, headers: (opts && opts.headers) || {}, body: (function(){ try { return JSON.parse((opts&&opts.body)||'{}'); } catch(_){ return {}; } })() }); } catch (_) {} return Promise.resolve({ status: 500, ok: false, json: function () { return Promise.resolve({ ok: false, error: 'server_error' }); } }); }
