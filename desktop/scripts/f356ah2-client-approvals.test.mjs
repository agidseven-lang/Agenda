/* F3.5.6A-H2 — CENTRAL DE APROVAÇÕES COMPACTA + DRAWER (Desktop 1.0.241) — suíte de CÓDIGO.
...
ok("A1 versão 1.0.241", pkg.version === "1.0.241", pkg.version);
...
try { const lock = JSON.parse(read(LOCK)); ok("A3 lock 1.0.241 ×2", lock.version === "1.0.241" && lock.packages[""].version === "1.0.241"); }
catch (e) { ok("A3 lock 1.0.241 ×2", false, String(e.message)); }
