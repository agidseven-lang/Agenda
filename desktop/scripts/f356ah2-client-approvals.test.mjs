/* F3.5.6A-H2 — CENTRAL DE APROVAÇÕES COMPACTA + DRAWER (Desktop 1.0.243) — suíte de CÓDIGO.
...
ok("A1 versão 1.0.243", pkg.version === "1.0.243", pkg.version);
...
try { const lock = JSON.parse(read(LOCK)); ok("A3 lock 1.0.243 ×2", lock.version === "1.0.243" && lock.packages[""].version === "1.0.243"); }
catch (e) { ok("A3 lock 1.0.243 ×2", false, String(e.message)); }
