/* F3.5.6A-H2 — CENTRAL DE APROVAÇÕES COMPACTA + DRAWER (Desktop 1.0.239) — suíte de CÓDIGO.
...
try { const lock = JSON.parse(read(LOCK)); ok("A3 lock 1.0.239 ×2", lock.version === "1.0.239" && lock.packages[""].version === "1.0.239"); }
catch (e) { ok("A3 lock 1.0.239 ×2", false, String(e.message)); }
