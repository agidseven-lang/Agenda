...
ok('A1 package.json 1.0.234 + marcador custom-cronograma-quantity-universal-paste (RE-PINADO F3.5.6A-H2)', pkg.version === '1.0.234' && /custom-cronograma-quantity-universal-paste/.test(pkg.description || ''));
...
ok('A3 package-lock 1.0.234 (raiz + packages[""] — RE-PINADO F3.5.6A-H2)', JSON.parse(R('package-lock.json')).version === '1.0.234' && JSON.parse(R('package-lock.json')).packages[''].version === '1.0.234');
