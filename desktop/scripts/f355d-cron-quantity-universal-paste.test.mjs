...
ok('A1 package.json 1.0.223 + marcador custom-cronograma-quantity-universal-paste', pkg.version === '1.0.223' && /custom-cronograma-quantity-universal-paste/.test(pkg.description || ''));
...
ok('A3 package-lock 1.0.223 (raiz + packages[""])', JSON.parse(R('package-lock.json')).version === '1.0.223' && JSON.parse(R('package-lock.json')).packages[''].version === '1.0.223');
