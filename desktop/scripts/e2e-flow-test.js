 ...
 * Agenda ID Seven · Worker V64.38-wa-card-link-cta / Desktop 1.0.135-beta-realtime-fix-v64-49-test / Android 1.0.135 (paridade)
 * ---------------------------------------------------------------------
...
console.log(' Worker V64.38-wa-card-link-cta · Desktop 1.0.135 · Android 1.0.135 (task-tabs-kanban-only: abas só no Kanban)');
console.log(`========================================================================${C.x}`);
...
check("D1", "package.json versão 1.0.135", /"version":\s*"1.0.135"/.test(DP));
...
check("D20", "Rodapé mostra Desktop 1.0.135", /Desktop 1\.0\.135/.test(DH));
// CORREÇÃO crítica (teste real): rótulo de versão do LOGIN não pode ficar defasado.
check("D21", "Login/título/watermark mostram 1.0.135 (sem rótulo antigo)",
  /<span class="pill-ver">Desktop 1\.0\.135/.test(DH) && /<title>ID Seven · Desktop 1\.0\.135/.test(DH) && /id="wpbadge">Desktop 1\.0\.135/.test(DH));
check('D22', 'Login espelha o APK 1.0.135-beta-realtime-fix-v64-49-test', /espelha o APK <b>1.0.135-beta-realtime-fix-v64-49-test/.test(DH));
check('D23', 'Fonte única APP_VER define a versão exibida', /const APP_VER=\{\s*desktop:.1\.0\.135./.test(DH) && /applyVersionLabels/.test(DH));
...
check('N1', 'versionName 1.0.135-beta-realtime-fix-v64-49-test (paridade Desktop)', /versionName\s+"1.0.135-beta-realtime-fix-v64-49-test"/.test(GRADLE));
...
check('N_FLOW4', 'Android versionName = 1.0.135-beta-realtime-fix-v64-49-test', /versionName "1.0.135-beta-realtime-fix-v64-49-test"/.test(GRADLE));
...
if (BLOCKING === 0) {
  ...
  console.log(`${C.g} Liberado para buildar DESKTOP 1.0.135-beta-realtime-fix-v64-49-test. Android segue por paridade após o Desktop.${C.x}`);
  console.log(`${C.b}========================================================================${C.x}`);
  process.exit(0);
} else {
