...
check("D1", "package.json versão 1.0.134", /"version":\s*"1.0.134"/.test(DP));
...
check("D20", "Rodapé mostra Desktop 1.0.134", /Desktop 1\.0\.134/.test(DH));
// CORREÇÃO crítica (teste real): rótulo de versão do LOGIN não pode ficar defasado.
check("D21", "Login/título/watermark mostram 1.0.134 (sem rótulo antigo)",
  /<span class="pill-ver">Desktop 1\.0\.134/.test(DH) && /<title>ID Seven · Desktop 1\.0\.134/.test(DH) && /id="wpbadge">Desktop 1\.0\.134/.test(DH));
check('D22', 'Login espelha o APK 1.0.134-beta-client-flow-v64-48-test', /espelha o APK <b>1.0.134-beta-client-flow-v64-48-test/.test(DH));
check('D23', 'Fonte única APP_VER define a versão exibida', /const APP_VER=\{\s*desktop:.1\.0\.134./.test(DH) && /applyVersionLabels/.test(DH));
...
check('N1', 'versionName 1.0.134-beta-client-flow-v64-48-test (paridade Desktop)', /versionName\s+"1.0.134-beta-client-flow-v64-48-test"/.test(GRADLE));
...
check('N_FLOW4', 'Android versionName = 1.0.134-beta-client-flow-v64-48-test', /versionName "1.0.134-beta-client-flow-v64-48-test"/.test(GRADLE));
