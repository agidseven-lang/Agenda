...
(function(){
  ...
  check('CY7_KCOL_AUTO','Colunas de ALTURA IGUAL (referência) + 1 card inteiro sem scrollbar + scrollbar fina + chrome enxuto',
    ...
    /#content\.board-mode \.kcol \.kbody\{padding:16px 10px 4px;scroll-snap-type:y proximity;scroll-padding-top:14px\}/.test(DH) && /#content\.board-mode \.kcol \.kbody \.tc:last-child\{margin-bottom:0\}/.test(DH));
  check('CY9_PROPORCAO_STATUS','Escopo limpo (reprovação 1.0.144): ZERO zoom/scale global (fitZoom removido) + colunas abraçam conteúdo (proporção em qualquer tela) + status nunca truncado + presença no canto do avatar',
    ...
    DH.indexOf("zoom:1.28")<0 && !/\.kanban\{zoom/.test(DH) && /INFLAÇÃO NEUTRALIZADA/.test(DH) && /\.tcv4-avw \.av\{width:64px/.test(DH)===false && /\.kanban\{display:flex;flex:1 1 auto;/.test(DH) && /\.kcol\{height:100%;max-height:100%;min-height:380px\}/.test(DH) &&
