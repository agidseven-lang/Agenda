 ...
 * Agenda ID Seven · Worker V64.38-wa-card-link-cta / Desktop 1.0.143-beta-design-contract-v1 / Android 1.0.143 (paridade)
 * ---------------------------------------------------------------------
...
check("D1", "package.json versão 1.0.143", /"version":\s*"1.0.143"/.test(DP));
...
check("D20", "Rodapé mostra Desktop 1.0.143", /Desktop 1\.0\.143/.test(DH));
// CORREÇÃO crítica (teste real): rótulo de versão do LOGIN não pode ficar defasado.
check("D21", "Login/título/watermark mostram 1.0.143 (sem rótulo antigo)",
  /<span class="pill-ver">Desktop 1\.0\.143/.test(DH) && /<title>ID Seven · Desktop 1\.0\.143/.test(DH) && /id="wpbadge">Desktop 1\.0\.143/.test(DH));
check('D22', 'Login espelha o APK 1.0.143-beta-design-contract-v1', /espelha o APK <b>1.0.143-beta-design-contract-v1/.test(DH));
check('D23', 'Fonte única APP_VER define a versão exibida', /const APP_VER=\{\s*desktop:.1\.0\.143./.test(DH) && /applyVersionLabels/.test(DH));
...
check('N1', 'versionName 1.0.143-beta-design-contract-v1 (paridade Desktop)', /versionName\s+"1.0.143-beta-design-contract-v1"/.test(GRADLE));
...
check('N_FLOW4', 'Android versionName = 1.0.143-beta-design-contract-v1', /versionName "1.0.143-beta-design-contract-v1"/.test(GRADLE));
...
(function(){
  ...
  check('CY2_CSS_KCOL','Card cortado (fix ESTRUTURAL V64.53): board-mode flex + coluna height:auto/max-height:100% (1 card = inteiro, sem scrollbar)',
    /#content\.board-mode\{display:flex;flex-direction:column;align-items:center;overflow:hidden;height:calc\(100vh\/var\(--fitz,1\)\)/.test(DH) && /#content\.board-mode \.kanban \.kcol\{height:auto;max-height:100%;min-height:0\}/.test(DH));
...
if (BLOCKING === 0) {
  ...
  console.log(`${C.g} Liberado para buildar DESKTOP 1.0.143-beta-design-contract-v1. Android segue por paridade após o Desktop.${C.x}`);
  console.log(`${C.b}========================================================================${C.x}`);
  process.exit(0);
} else {
