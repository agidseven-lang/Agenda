...
check("D20", "Rodapé mostra Desktop 1.0.146", /Desktop 1\.0\.146/.test(DH));
// CORREÇÃO crítica (teste real): rótulo de versão do LOGIN não pode ficar defasado.
check("D21", "Login/título/watermark mostram 1.0.146 (sem rótulo antigo)",
  /<span class="pill-ver">Desktop 1\.0\.146/.test(DH) && /<title>ID Seven · Desktop 1\.0\.146/.test(DH) && /id="wpbadge">Desktop 1\.0\.146/.test(DH));
check('D22', 'Login espelha o APK 1.0.146-beta-board-rebuild', /espelha o APK <b>1.0.146-beta-board-rebuild/.test(DH));
check('D23', 'Fonte única APP_VER define a versão exibida', /const APP_VER=\{\s*desktop:.1\.0\.146./.test(DH) && /applyVersionLabels/.test(DH));
...
(function(){
  ...
  check('CY5_KANBAN_BOARDMODE','board-mode (referência): flex gap 15, altura IGUAL, coluna EM FOCO 1.16x mais larga (sem teto: preenche a largura)',
  ...
  check('CY9_PROPORCAO_STATUS','Escopo limpo (reprovação 1.0.144): ZERO zoom/scale global (fitZoom removido) + colunas abraçam conteúdo (proporção em qualquer tela) + status nunca truncado + presença no canto do avatar',
    ...
    DH.indexOf("zoom:1.28")<0 && !/\.kanban\{zoom/.test(DH) && /@media\(min-width:1700px\)/.test(DH) && /\.kcol\{min-width:300px;max-width:none;min-height:470px;border-radius:12px\}/.test(DH) && /\.tcv4-title\{font-size:19px\}/.test(DH) && /\.kanban\{display:flex;flex:1 1 auto;/.test(DH) && /\.kcol\{height:100%;max-height:100%;min-height:380px\}/.test(DH) &&
  ...
  check('CY8_LAUDO_141','Laudo 1.0.141 + 1.0.146-rec: perfil flex:none (não estica) + colunas PREENCHEM a largura (max-width:none, autorizado) + popover ⋯ FIXED ancorado (nunca cortado)',
