...
ok('A1 package.json 1.0.239 (RE-PINADO F3.5.6A-H2; base traz a Agenda Premium)', pkg.version === '1.0.239' && /custom-script-quantity-rich-editor/.test(pkg.description || ''));
ok('A2 package-lock 1.0.239 (RE-PINADO F3.5.6A-H2)', JSON.parse(R('package-lock.json')).version === '1.0.239');
ok('A3 lock packages[""] 1.0.239 (RE-PINADO F3.5.6A-H2)', JSON.parse(R('package-lock.json')).packages[''].version === '1.0.239');
