...
(async () => {
  ...
  // 2) token/ticket NUNCA no estado nem nos states empurrados ao renderer
  {
    ...
    const allowed = ['phase', 'wsConnected', 'authValidated', 'service', 'lastConnectAt', 'lastMessageAt', 'heartbeatActive', 'baselineReceived', 'onlineCount', 'wsEnabled', 'errorCode',
      'wantConnected', 'lastHeartbeatSentAt', 'lastCloseCode', 'lastCloseReason', 'lastCloseWasClient', 'reconnectScheduled', 'reconnectExecuted', 'onlineDroppedAt', 'roster'].sort();
    ok('2 chaves do estado == conjunto sanitizado exato (incl. diagnóstico Stage-2A-X2 + roster Stage-2C)', JSON.stringify(Object.keys(c.getState()).sort()) === JSON.stringify(allowed));
    ok('2 roster é array e cada entrada NÃO tem deviceId (sanitizado)', Array.isArray(c.getState().roster) && c.getState().roster.every((e) => !('deviceId' in e)));
