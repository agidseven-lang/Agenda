# RUNBOOK HUMANO v2 (4.4-A4-PREP-v2) — BASELINE REAL READ-ONLY
...
## ⚠️ AVISO CENTRAL
O deploy do V64.7, mesmo read-only, é uma ALTERAÇÃO REAL do Worker de produção.
...
- Hashes de referência:
  - V64.5 produção: `58264a3da232348269b1699a9d80e39e4e3db1477ce0df2ec092df62fb3e661f`
...
A equivalência dessas rotas V64.5×V64.7 já foi provada em harness (15/15) — não há
...
A2. Comando C1 → resposta deve conter `"version":"V64.5-designer-notify"`.
A3. Settings → Variables and Secrets → PRINT da lista. Estado esperado: **zero `SLA_*`**.
A4. BACKUP: "Edit code" → selecionar TODO o código → salvar como `worker-v645-backup.js`
    → conferir hash (C2) = `58264a3d…fb3e661f`.
A5. PACOTE: baixar `cloudflare-worker.js` do branch `desktop/local-detail-hierarchy-v2`
    (raw do GitHub) como `worker-v647.js` → conferir hash (C3) = `b17a5f37…b8cf7acc`.
...
[ ] A2 = V64.5 confirmado   [ ] A3 = zero `SLA_*` (print salvo)
...
B1. "Edit code" → apagar conteúdo → colar `worker-v647.js` INTEIRO.
B2. "Save and deploy". (Anotar horário.)
B3. C1 → `"version":"V64.7-sla-baseline-ready"`.
...
E6. DECISÃO DE PERMANÊNCIA (regra de encerramento):
    - Opção 1: manter V64.7 SEM env (comportamento byte-equivalente ao V64.5 —
      provado; deixa o terreno pronto para a 4.4-B);
    - Opção 2 (conservação máxima): restaurar V64.5 via seção 6 nível 2.
...
C2  sha256sum worker-v645-backup.js
C3  sha256sum worker-v647.js
...
1. Hash do backup ≠ `58264a3d…fb3e661f` (produção não é o V64.5 esperado).
2. Hash do pacote ≠ `b17a5f37…b8cf7acc` (arquivo errado/corrompido).
3. C1 inicial não responder `V64.5-designer-notify`.
...
- **Nível 2 (restauração total):** "Edit code" → colar `worker-v645-backup.js`
  (hash conferido em A4) → "Save and deploy" → C1 = `V64.5-designer-notify`.

## 7. CHECKLIST FINAL (antes / durante / depois)
ANTES:   [ ] horário início anotado [ ] A2 V64.5 [ ] A3 zero SLA_* (print)
         [ ] A4 backup hash OK [ ] A5 pacote hash OK [ ] PONTO DE PARADA cumprido
DURANTE: [ ] B3 V64.7 [ ] B4 403/403 [ ] B5 zero env pós-deploy (print)
...
DEPOIS:  [ ] E1 env removida [ ] E2 403 (print) [ ] E3 zero SLA_* (print)
         ...
         [ ] E6 decisão V64.7-sem-env OU V64.5 anotada [ ] E7 horário fim anotado
